...
namespace MonoTests.System.Net.Http {
	...
	public class MessageHandlerTest {
		...
		public void TestNSUrlSessionHandlerSendClientCertificate ()
		{
			...
			// Use the ephemeral keystore so that the test do pass on macOS, otherwise we will get an exception from dotnet because it does not have the rights 
			// to access the defualt key storage.
			// ref: https://learn.microsoft.com/en-us/dotnet/api/system.security.cryptography.x509certificates.x509keystorageflags?view=net-8.0
			X509Certificate2 certificate = new X509Certificate2 (global::System.Convert.FromBase64String (certificate_base64), "test", X509KeyStorageFlags.EphemeralKeySet);
			string content = "";
			var done = TestRuntime.TryRunAsync (TimeSpan.FromSeconds (30), async () => {

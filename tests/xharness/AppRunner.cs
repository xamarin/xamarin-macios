		...
		public async Task<int> RunAsync ()
		{
			...
			if (!success.Value) {
				...
				if (!string.IsNullOrEmpty (crash_reason)) {
					...
					if (Harness.InCI)
						XmlResultParser.GenerateFailure (Logs, "launch", appName, Variation, $"AppLaunch on {device_name}", $"{FailureMessage} on {device_name}", main_log.FullPath, Harness.XmlJargon);

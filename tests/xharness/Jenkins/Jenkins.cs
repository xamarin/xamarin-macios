...
namespace Xharness.Jenkins {
	public class Jenkins : IResourceManager
	{
		...
		List<JenkinsTestTask> Tasks = new List<JenkinsTestTask> ();
		...
		async Task<IEnumerable<JenkinsTestTask>> CreateRunSimulatorTasksAsync ()
		{
		...
		Task<IEnumerable<JenkinsTestTask>> CreateRunDeviceTasksAsync ()
		{
			...
			return Task.FromResult<IEnumerable<JenkinsTestTask>> (CreateTestVariations (rv, (buildTask, test, candidates) => new RunDeviceTask (devices, buildTask, processManager, candidates?.Cast<IHardwareDevice> () ?? test.Candidates)));
		}
			...
			var thread = new System.Threading.Thread (() =>
			{
				while (server.IsListening) {
					...
					try {
						...
						{
							var rv = new List<JenkinsTestTask> ();
						...
						IEnumerable<JenkinsTestTask> find_tasks (StreamWriter writer, string ids)
						{
							IEnumerable<JenkinsTestTask> tasks;
							...
							case "?":
								...
								return Array.Empty<JenkinsTestTask> ();
							default:
								var id_inputs = ids.Substring (1).Split (',');
								var rv = new List<JenkinsTestTask> (id_inputs.Length);
		...
		string GetTestColor (IEnumerable<JenkinsTestTask> tests)
		{
			if (!tests.Any ())
				return "black";
		...
		string GetTestColor (JenkinsTestTask test)
		{
			if (test.NotStarted) {
				return "black";
		...
		void GenerateReportImpl (Stream stream, StreamWriter markdown_summary = null)
		{
			...
			var allTasks = new List<JenkinsTestTask> ();
			if (!populating) {
	...
	");
				...
				var orderedTasks = allTasks.GroupBy ((JenkinsTestTask v) => v.TestName);
				...
				if (IsServerMode) {
					...
					if (failedTests.Count () == 0) {
						foreach (var group in failedTests.GroupBy ((v) => v.TestName)) {
							var enumerableGroup = group as IEnumerable<JenkinsTestTask>;
		...
		string RenderTextStates (IEnumerable<JenkinsTestTask> tests)
		{
			...
			IEnumerable<JenkinsTestTask> relevantGroup;
			if (allIgnored) {
				relevantGroup = tests;
			} else {

...
namespace Xharness.Jenkins
{
	public class Jenkins
	{
		...
		Task PopulateTasksAsync ()
		{
			...
			{
				Jenkins = this,
				TestProject = new TestProject (Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "..", "msbuild", "tests", "Xamarin.iOS.Tasks.Tests", "Xamarin.iOS.Tasks.Tests.csproj"))),
				SpecifyPlatform = false,
				SpecifyConfiguration = true,
				ProjectConfiguration = "Debug-net461",
				Platform = TestPlatform.iOS,
				SolutionPath = Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "..", "msbuild", "Xamarin.MacDev.Tasks.sln")),
				SupportsParallelExecution = false,
			};
			var nunitExecutioniOSMSBuild_net461 = new NUnitExecuteTask (buildiOSMSBuild_net461)
			{
				TestLibrary = Path.Combine (DirectoryUtilities.RootDirectory, "..", "msbuild", "tests", "Xamarin.iOS.Tasks.Tests", "bin", "Debug-net461", "net461", "Xamarin.iOS.Tasks.Tests.dll"),
			...
			var nunitExecutioniOSMSBuild_netstandard2 = new NUnitExecuteTask (buildiOSMSBuild_netstandard2) {
				TestLibrary = Path.Combine (DirectoryUtilities.RootDirectory, "..", "msbuild", "tests", "Xamarin.iOS.Tasks.Tests", "bin", "Debug-netstandard2.0", "net461", "Xamarin.iOS.Tasks.Tests.dll"),
			...
			{
				Jenkins = this,
				TestProject = new TestProject (Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "..", "tools", "install-source", "InstallSourcesTests", "InstallSourcesTests.csproj"))),
				SpecifyPlatform = false,
				SpecifyConfiguration = false,
				Platform = TestPlatform.iOS,
			};
			buildInstallSources.SolutionPath = Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "..", "tools", "install-source", "install-source.sln")); // this is required for nuget restore to be executed
			var nunitExecutionInstallSource = new NUnitExecuteTask (buildInstallSources)
			{
				TestLibrary = Path.Combine (DirectoryUtilities.RootDirectory, "..", "tools", "install-source", "InstallSourcesTests", "bin", "Release", "InstallSourcesTests.dll"),
			...
			{
				Jenkins = this,
				TestProject = new TestProject (Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "mtouch", "mtouch.sln"))),
				SpecifyPlatform = false,
				SpecifyConfiguration = false,
				Platform = TestPlatform.iOS,
				Target = "dependencies",
				WorkingDirectory = Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "mtouch")),
			};
			var nunitExecutionMTouch = new NUnitExecuteTask (buildMTouch)
			{
				TestLibrary = Path.Combine (DirectoryUtilities.RootDirectory, "mtouch", "bin", "Debug", "mtouch.dll"),
				TestProject = new TestProject (Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "mtouch", "mtouch.csproj"))),
				Platform = TestPlatform.iOS,
			...
			var buildGenerator = new MakeTask {
				Jenkins = this,
				TestProject = new TestProject (Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "..", "src", "generator.sln"))),
				SpecifyPlatform = false,
				SpecifyConfiguration = false,
				Platform = TestPlatform.iOS,
				Target = "build-unit-tests",
				WorkingDirectory = Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "generator")),
			};
			var runGenerator = new NUnitExecuteTask (buildGenerator) {
				TestLibrary = Path.Combine (DirectoryUtilities.RootDirectory, "generator", "bin", "Debug", "generator-tests.dll"),
				TestProject = new TestProject (Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "generator", "generator-tests.csproj"))),
				Platform = TestPlatform.iOS,
			...
			var buildDotNetGenerator = new DotNetBuildTask {
				Jenkins = this,
				TestProject = new TestProject (Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "bgen", "bgen-tests.csproj"))),
				SpecifyPlatform = false,
				SpecifyConfiguration = false,
				Platform = TestPlatform.iOS,
			};
			...
			var run_mmp = new MakeTask
			{
				...
				WorkingDirectory = Path.Combine (DirectoryUtilities.RootDirectory, "mmptest", "regression"),
			...
			var runMacBindingProject = new MakeTask
			{
				...
				Target = "all",
				WorkingDirectory = Path.Combine (DirectoryUtilities.RootDirectory, "mac-binding-project"),
			...
			var buildXtroTests = new MakeTask {
				...
				TestName = "Xtro",
				Target = "wrench",
				WorkingDirectory = Path.Combine (DirectoryUtilities.RootDirectory, "xtro-sharpie"),
				Ignored = !IncludeXtro,
				Timeout = TimeSpan.FromMinutes (15),
			};
			...
			var buildCecilTests = new MakeTask {
				...
				TestName = "Cecil",
				Target = "build",
				WorkingDirectory = Path.Combine (DirectoryUtilities.RootDirectory, "cecil-tests"),
			...
			var runDocsTests = new MakeTask {
				...
				Target = "wrench-docs",
				WorkingDirectory = DirectoryUtilities.RootDirectory,
				Ignored = !IncludeDocs,
			...
			var buildSampleTests = new MSBuildTask {
				Jenkins = this,
				TestProject = new TestProject (Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "sampletester", "sampletester.sln"))),
				SpecifyPlatform = false,
				Platform = TestPlatform.All,
				ProjectConfiguration = "Debug",
			};
			var runSampleTests = new NUnitExecuteTask (buildSampleTests) {
				TestLibrary = Path.Combine (DirectoryUtilities.RootDirectory, "sampletester", "bin", "Debug", "sampletester.dll"),
				TestProject = new TestProject (Path.GetFullPath (Path.Combine (DirectoryUtilities.RootDirectory, "sampletester", "sampletester.csproj"))),
				Platform = TestPlatform.All,
		...
		Task BuildTestLibrariesAsync ()
		{
			...
			return Harness.ProcessManager.ExecuteCommandAsync ("make", new [] { "all", $"-j{Environment.ProcessorCount}", "-C", Path.Combine (DirectoryUtilities.RootDirectory, "test-libraries") }, log, TimeSpan.FromMinutes (10)).ContinueWith ((v) => {
				var per = v.Result;
				if (!per.Succeeded) {
				...
				while (server.IsListening) {
					...
					try {
						...
						case "/quit":
							...
							break;
						case "/favicon.ico":
							serveFile = Path.Combine (DirectoryUtilities.RootDirectory, "xharness", "favicon.ico");
							goto default;
						case "/index.html":
		...
		public void GenerateReport ()
		{
			try {
				lock (report_lock) {
					...
					File.Copy (Path.Combine (DirectoryUtilities.RootDirectory, "xharness", "favicon.ico"), Path.Combine (LogDirectory, "favicon.ico"), true);
				}
			} catch (Exception e) {

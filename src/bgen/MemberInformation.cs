...
public class MemberInformation {
	...
	MemberInformation (Generator generator, IMemberGatherer gather, MemberInfo mi, Type type, bool is_interface_impl, bool is_extension_method, bool is_appearance, bool is_model)
	{
		...
		if (mi.DeclaringType != type) {
			for (var baseType = ReflectionExtensions.GetBaseType (type, generator); baseType is not null && baseType != Generator.TypeManager.System_Object; baseType = ReflectionExtensions.GetBaseType (baseType, generator)) {
		...
		: this (generator, gather, mi, type, isInterfaceImpl, isExtensionMethod, isAppearance, isModel)
	{
		...
		foreach (ParameterInfo pi in mi.GetParameters ())
			if (pi.ParameterType.IsSubclassOfNotNullable (Generator.TypeManager.System_Delegate))
				is_unsafe = true;

		if (!is_unsafe && mi.ReturnType.IsSubclassOfNotNullable (Generator.TypeManager.System_Delegate))
			is_unsafe = true;
	...
	{
		if (pi.PropertyType.IsSubclassOfNotNullable (Generator.TypeManager.System_Delegate))
			is_unsafe = true;

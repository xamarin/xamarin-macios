...
{
	class MobileAuthenticatedStream : AuthenticatedStream, IMonoSslStream
	{
		AppleTlsContext xobileTlsContext;
		...
		internal AppleTlsContext Context {
			get {
				CheckThrow (true);
		...
		internal int InnerRead (int requestedSize)
		{
			...
			var len = Math.Min (readBuffer.Remaining, requestedSize);
			if (len == 0)
				throw new InvalidOperationException ();

		...
		internal class ObjCType {
			...
			public void VerifyRegisterAttribute (ref List<Exception> exceptions)
			{
				...
				for (int i = 0; i < name.Length; i++) {
					...
					AddException (ref exceptions, new ProductException (4146, showError, Errors.mtouch.MT4146, Registrar.GetTypeFullName (Type), name [i], ((int) name [i]).ToString ("x"), name));
					break;
				}
			}
			...
			void VerifySelector (ObjCMethod method, ref List<Exception> exceptions)
			{
				...
				if (nativeParamCount != paramCount) {
					...
					if (method.IsVariadic) {
						ex = Registrar.CreateException (4140, method, Errors.mtouch.MT4140,
							method.Method.DeclaringType.FullName, method.MethodName, nativeParamCount, paramCount, method.Selector);
					} else {
						ex = Registrar.CreateException (4117, method, Errors.mtouch.MT4117,
							method.Method.DeclaringType.FullName, method.MethodName, nativeParamCount, paramCount, method.Selector);
					}
				...
				if (method.IsVariadic && pars != null && Registrar.GetTypeFullName (pars [paramCount - 1]) != "System.IntPtr")
					Registrar.AddException (ref exceptions, Registrar.CreateException (4123, method, Errors.mtouch.MT4123, Registrar.GetDescriptiveMethodName (method.Method)));
				
				char ch;
				var idx = method.Selector.IndexOfAny (invalidSelectorCharacters);
				if (idx != -1) {
					ch = method.Selector [idx];
					Registrar.AddException (ref exceptions, Registrar.CreateException (4160, method, Errors.mtouch.MT4160,
			...
			void VerifyIsNotKeyword (ref List<Exception> exceptions, ObjCProperty property)
			{
				if (IsObjectiveCKeyword (property.Selector))
					AddException (ref exceptions, CreateException (4164, property, Errors.mtouch.MT4164, property.Name, property.Selector));
			}
			...
			bool AddToMap (ObjCMember member, ref List<Exception> exceptions)
			{
				...
				if (TryGetMember (member.Selector, member.IsNativeStatic, out existing)) {
					if (existing.IsImplicit) {
						AddException (ref exceptions, CreateException (4141, member, Errors.mtouch.MT4141, member.Selector, Registrar.GetTypeFullName (Type), Registrar.GetMemberName (member)));
					} else {
						AddException (ref exceptions, CreateException (4119, member, Errors.mtouch.MT4119, member.Selector, Registrar.GetTypeFullName (Type), Registrar.GetMemberName (member), Registrar.GetMemberName (existing)));
					}
					rv = false;
				}
		...
		abstract internal class ObjCMember {
			...
			public bool SetExportAttribute (ExportAttribute ea, ref List<Exception> exceptions)
			{
				if (string.IsNullOrEmpty (ea.Selector)) {
					AddException (ref exceptions, Registrar.CreateException (4135, this, Errors.mtouch.MT4135, FullName));
					return false;
				}
			...
			public string Selector {
				...
				set {
					if (string.IsNullOrEmpty (value))
						throw Registrar.CreateException (4135, this, Errors.mtouch.MT4135, FullName);
					selector = value;
				}
			}
						...
						for (int i = 0; i < parameters.Length; i++) {
							...
							if (originalType != null) {
								if (!IsValidToManagedTypeConversion (originalType, parameters [i]))
									throw Registrar.CreateException (4172, Method, Errors.mtouch.MT4172, Registrar.GetTypeFullName (parameters [i]),  originalType.FullName, Registrar.GetParameterName (Method, i), DescriptiveMethodName);
						...
						if (Registrar.Is (ReturnType, "System", "Void")) {
							...
							if (originalType != null) {
								if (!IsValidToManagedTypeConversion (originalType, ReturnType))
									throw Registrar.CreateException (4170, Method, Errors.mtouch.MT4170, Registrar.GetTypeFullName (ReturnType), originalType.FullName, DescriptiveMethodName);
...
#if MTOUCH || MMP
					throw ErrorHelper.CreateError (8018, Errors.mtouch.MT8018);
#else
		...
		public BindAsAttribute GetBindAsAttribute (ObjCMethod method, int parameter_index)
		{
			...
			if (attrib != null) {
				...
				if (parameter_index == -1) {
					...
					if (!AreEqual (returnType, attrib.Type))
						throw CreateException (4171, method.Method, Errors.mtouch.MT4171, method.DescriptiveMethodName, GetTypeFullName (attrib.Type), GetTypeFullName (returnType));
				} else {
					...
					if (!AreEqual (parameterType, attrib.Type))
						throw CreateException (4171, method.Method, Errors.mtouch.MT4171_A, parameter_index + 1, GetTypeFullName(attrib.Type), GetTypeFullName(parameterType));
				}
			...
			if (attrib != null) {
				...
				if (!AreEqual (propertyType, attrib.Type))
					throw CreateException (4171, property, Errors.mtouch.MT4171_B, GetTypeFullName (method.DeclaringType.Type), GetPropertyName (property),  GetTypeFullName (attrib.Type), GetTypeFullName (propertyType));
			}
			return attrib;
		}
...
#error Unknown platform
		...
		bool VerifyIsConstrainedToNSObject (ref List<Exception> exceptions, TType type, ObjCMethod method)
		{
			...
			if (!VerifyIsConstrainedToNSObject (method.ReturnType, out constrained_type)) {
				AddException (ref exceptions, CreateException (4129, method.Method, Errors.mtouch.MT4129, GetTypeFullName (method.ReturnType), GetDescriptiveMethodName (type, method.Method)));
				return false;
			}
			...
			for (int i = 0; i < pars.Length; i++) {
				...
				if (!VerifyIsConstrainedToNSObject (p, out constrained_type)) {
					AddException (ref exceptions, CreateException (4128, method.Method, Errors.mtouch.MT4128, GetTypeFullName (p), GetDescriptiveMethodName (type, method.Method), GetParameterName (method.Method, i)));
					return false;
				}
				if (constrained_type != null) {
					if (types == null) {
		...
		ObjCType RegisterCategory (TType type, CategoryAttribute attrib, ref List<Exception> exceptions)
		{
			if (IsINativeObject (type)) {
				AddException (ref exceptions, ErrorHelper.CreateError (4152, Errors.mtouch.MT4152, GetTypeFullName (type)));
				return null;
			}

			if (IsGenericType (type)) {
				AddException (ref exceptions, ErrorHelper.CreateError (4153, Errors.mtouch.MT4153, GetTypeFullName (type)));
				return null;
			}

			if (attrib.Type == null) {
				AddException (ref exceptions, ErrorHelper.CreateError (4151, Errors.mtouch.MT4151, GetTypeFullName (type)));
				return null;
			}

			var declaringType = RegisterType (attrib.Type, ref exceptions);
			if (declaringType == null) {
				AddException (ref exceptions, ErrorHelper.CreateError (4150, Errors.mtouch.MT4150, GetTypeFullName (type), GetTypeFullName (attrib.Type)));
				return null;
			}
			...
			lock (categories_map) {
				...
				if (categories_map.TryGetValue (objcType.CategoryName, out previous_type)) {
					AddException (ref exceptions, ErrorHelper.CreateError (4156, Errors.mtouch.MT4156,
			...
			foreach (var ctor in CollectConstructors (type)) {
				...
				AddException (ref exceptions, CreateException (4158, ctor, Errors.mtouch.MT4158, GetTypeFullName (type), GetDescriptiveMethodName (ctor)));
			}
			...
			foreach (var method in CollectMethods (type)) {
				...
				if (!IsStatic (method)) {
					AddException (ref exceptions, CreateException (4159, method, Errors.mtouch.MT4159, GetTypeFullName (type), GetMethodName (method)));
					return null;
				}
				...
				if (HasThisAttribute (method)) {
					...
					if (parameters == null || parameters.Length == 0) {
						AddException (ref exceptions, CreateException (4157, method, Errors.mtouch.MT4157,
						...
						AddException (ref exceptions, CreateException (4149, method, Errors.mtouch.MT4149,
				...
				if (IsGenericMethod (method)) {
					AddException (ref exceptions, CreateException (4154, method, Errors.mtouch.MT4154, GetTypeFullName (type), GetMethodName (method)));
					continue;
				}
		...
		ObjCType RegisterTypeUnsafe (TType type, ref List<Exception> exceptions)
		{
			...
			if (!IsNSObject (type)) {
				...
				if (isGenericType) {
					exceptions.Add (ErrorHelper.CreateError (4148, Errors.mtouch.MT4148, GetTypeFullName (type)));
					return null;
				}
			...
			if (ObjCType.IsObjectiveCKeyword (objcType.ExportedName))
				AddException (ref exceptions, ErrorHelper.CreateError (4168, Errors.mtouch.MT4168,  GetTypeFullName (type), objcType.ExportedName));
			...
			if (objcType.IsProtocol) {
				lock (protocol_map) {
					if (protocol_map.TryGetValue (objcType.ExportedName, out previous_type))
						throw ErrorHelper.CreateError (4126, Errors.mtouch.MT4126,
				...
				lock (type_map) {
					if (type_map.TryGetValue (objcType.ExportedName, out previous_type))
						throw ErrorHelper.CreateError (4118, Errors.mtouch.MT4118,
							GetAssemblyQualifiedName (type), GetAssemblyQualifiedName (previous_type), objcType.ExportedName);
			...
			foreach (TProperty property in properties) {
				...
				if (IsStatic (property) && isGenericType) {
					AddException (ref exceptions, CreateException (4131, property, Errors.mtouch.MT4131, GetTypeFullName (type), GetPropertyName (property)));
					continue;
				}

				TType property_type = null;
				if (isGenericType && !VerifyIsConstrainedToNSObject (GetPropertyType (property), out property_type)) {
					AddException (ref exceptions, CreateException (4132, property, Errors.mtouch.MT4132, GetTypeFullName (GetPropertyType (property)), GetTypeFullName (type), GetPropertyName (property)));
					continue;
				}
				if (property_type == null)
				...
				if (getter != null && VerifyNonGenericMethod (ref exceptions, type, getter)) {
					...
					if (!method.ValidateSignature (ref excs)) {
						exceptions.Add (CreateException (4138, excs [0], property, Errors.mtouch.MT4138,
			...
			foreach (TMethod method in methods) {
				...
				if (IsStatic (method) && isGenericType) {
					AddException (ref exceptions, CreateException (4130, method, Errors.mtouch.MT4130, GetDescriptiveMethodName (type, method)));
					continue;
		...
		public void RegisterAssembly (TAssembly assembly)
		{
			...
			try {
				...
				ReportError (4116, Errors.mtouch.MT4116, GetAssemblyName (assembly), e);
			}
		...
		public string ComputeSignature (TType declaring_type, bool is_ctor, TType return_type, TType [] parameters, TMethod mi = null, ObjCMember member = null, bool isCategoryInstance = false, bool isBlockSignature = false)
		{
			...
			if (is_ctor) {
				...
				if (!success)
					throw CreateException (4104, mi, Errors.mtouch.MT4104_A, GetTypeFullName (return_type), GetTypeFullName (declaring_type), GetDescriptiveMethodName (mi));
			}
			...
			if (parameters != null) {
				for (int i = 0; i < parameters.Length; i++) {
					...
					if (!success) {
						...
						throw CreateException (4136, mi, Errors.mtouch.MT4136,
		...
		protected string ToSignature (TType type, ObjCMember member, bool forProperty = false)
		{
			...
			if (objcMethod != null)
				throw ErrorHelper.CreateError (4111, Errors.mtouch.MT4111, GetTypeFullName (type), GetTypeFullName (objcMethod.DeclaringType.Type) + "." + objcMethod.MethodName);

			throw ErrorHelper.CreateError (4101, Errors.mtouch.MT4101, GetTypeFullName (type));
		}
		...
		protected string ToSignature (TType type, ObjCMember member, ref bool success, bool forProperty = false)
		{
			...
			case "System.DateTime":
				throw CreateException (4102, member, Errors.mtouch.MT4102, "System.DateTime", "Foundation.NSDate", member.FullName);
			}
			...
			if (IsINativeObject (type)) {
				if (!IsGenericType (type) && !IsInterface (type) && !IsNSObject (type) && IsAbstract (type))
					ErrorHelper.Show (CreateWarning (4179, member, Errors.mtouch.MT4179, type.FullName, member.FullName));
			...
			if (IsEnum (type, out isNativeEnum)) {
				if (isNativeEnum && !Is64Bits) {
					...
					case "System.UInt64":
						return "I";
					default:
						throw CreateException (4145, Errors.mtouch.MT4145, GetTypeFullName (type));
					}
				} else {

...
namespace XamCore.CallKit {
	...
	// FIXME: Should we put these constants somewhere else?
	[Static]
	[Introduced (PlatformName.iOS, 10, 0)]
	[Introduced (PlatformName.MacOSX, 10, 12)]
	interface CXErrorDomain {
		[Field ("CXErrorDomain")]
		NSString ErrorDomain { get; }

		[Field ("CXErrorDomainIncomingCall")]
		NSString IncomingCall { get; }

		[Field ("CXErrorDomainRequestTransaction")]
		NSString RequestTransaction { get; }

		[Field ("CXErrorDomainCallDirectoryManager")]
		NSString CallDirectoryManager { get; }
	}
	...
	interface CXProviderDelegate {
		...
		void StartCallAction (CXProvider provider, CXStartCallAction action);

		[Export ("provider:performAnswerCallAction:")]
		void AnswerCallAction (CXProvider provider, CXAnswerCallAction action);

		[Export ("provider:performEndCallAction:")]
		void EndCallAction (CXProvider provider, CXEndCallAction action);

		[Export ("provider:performSetHeldCallAction:")]
		void SetHeldCallAction (CXProvider provider, CXSetHeldCallAction action);

		[Export ("provider:performSetMutedCallAction:")]
		void SetMutedCallAction (CXProvider provider, CXSetMutedCallAction action);

		[Export ("provider:performSetGroupCallAction:")]
		void SetGroupCallAction (CXProvider provider, CXSetGroupCallAction action);

		[Export ("provider:performPlayDTMFCallAction:")]
		void PlayDtmfCallAction (CXProvider provider, CXPlayDtmfCallAction action);
	...
	interface CXProvider {
		...
		[Export ("setDelegate:queue:")]
		void SetDelegate ([NullAllowed] ICXProviderDelegate aDdelegate, [NullAllowed] DispatchQueue queue);

		...
		public void Convert ()
		{
			...
			case ApplePlatform.TVOS:
				if ((abi & Abi.ARM64) != 0) {
					...
					throw ErrorHelper.CreateError (1301, Errors.mtouch.MT1301, abi);
				}
				...
				break;
			default:
				throw ErrorHelper.CreateError (1300, Errors.mtouch.MT1300, platform);
			}
		...
		string FixAsciz (string s, int line) {
			...
			for (int i = 0; i < utf8.Length; ++i) {
				...
				if (utf8 [i] == '\\') {
					++i;
					if (i >= utf8.Length)
						throw ErrorHelper.CreateError (1302, Errors.mtouch.MT1302_A, input, line);
					switch (utf8 [i]) {
					case (byte) 'b':
						to_append = 0x8;
						break;
					...
					default:
						if (IsDigit (utf8 [i])) {
							...
							if (!ParseOctal (utf8, ref i, ref to_append, ref error_msg))
								throw ErrorHelper.CreateError (1302, Errors.mtouch.MT1302_B, input, line, error_msg);
						} else

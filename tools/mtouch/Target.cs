	...
	public partial class Target {
		...
		public void AddToBundle (string source, string bundle_path = null, bool dylib_to_framework_conversion = false)
		{
			...
			if (info.DylibToFramework != dylib_to_framework_conversion)
				throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_L);
			...
			get {
				if (executables == null) {
					...
					if (App.IsSimulatorBuild) {
						...
						if (Abis.Count != 1)
							throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_L, string.Join (", ", Abis.Select ((v) => v.ToString ())));
		...
		public void Initialize (bool show_warnings)
		{
			...
			if (corlib == null)
				throw new MonoTouchException (2006, true, Errors.mtouch.MT2006, corlib_path);
			...
			foreach (var root_assembly in App.RootAssemblies) {
				...
				if (root == null) {
					...
					throw ErrorHelper.CreateError (2019, Errors.mtouch.MT2019, root_assembly);
				}
				roots.Add (root);
			}
			...
			foreach (var reference in App.References) {
				...
				if (ad == null)
					throw new MonoTouchException (2002, true, Errors.mtouch.MT2002, reference);
				...
				if (root_assembly != null) {
					...
					throw ErrorHelper.CreateError (23, Errors.mtouch.MT0023, root_assembly.MainModule.FileName, reference);
				}
				
				if (ad.MainModule.Runtime > TargetRuntime.Net_4_0)
					ErrorHelper.Show (new MonoTouchException (11, false, Errors.mtouch.MT0011, Path.GetFileName (reference), ad.MainModule.Runtime));
				...
				if (ad != ProductAssembly && GetRealPath (filename) != GetRealPath (reference) && !filename.EndsWith (".resources.dll", StringComparison.Ordinal))
					ErrorHelper.Show (ErrorHelper.CreateWarning (109, Errors.mtouch.MT0109, Path.GetFileName (reference), reference, filename));
			}
			...
			if (!App.Embeddinator) {
				if (!Assemblies.Any ((v) => v.AssemblyDefinition.Name.Name == Driver.GetProductAssembly (App)))
					throw ErrorHelper.CreateError (123, Errors.mtouch.MT0123, App.RootAssemblies [0], Driver.GetProductAssembly (App));
			}
			...
			if (App.InterpretedAssemblies.Count > 0) {
				...
				foreach (var entry in App.InterpretedAssemblies) {
					...
					exceptions.Add (ErrorHelper.CreateWarning (142, Errors.mtouch.MT0142, assembly));
				}
		...
		public void ComputeListOfAssemblies ()
		{
			...
			try {
				...
				exceptions.Add (new MonoTouchException (9, true, e, Errors.mtouch.MT0009, e.Message));
			}
			...
			foreach (AssemblyNameReference reference in main.AssemblyReferences) {
				...
				case "Xamarin.WatchOS":
					if (reference.Name != Driver.GetProductAssembly (App))
						exceptions.Add (ErrorHelper.CreateError (34, Errors.mtouch.MT0034, reference.Name, Driver.TargetFramework.Identifier, assembly.FullName));
					break;
				}

				var reference_assembly = ManifestResolver.Resolve (reference);
				if (reference_assembly == null) {
					ErrorHelper.Warning (136, Errors.mtouch.MT0136, reference.FullName, main.FileName);
					continue;
				}
		...
		void GetCustomAttributeArgumentReference (ModuleDefinition main, CustomAttribute ca, CustomAttributeArgument arg, HashSet<string> assemblies, List<Exception> exceptions)
		{
			...
			if (reference_assembly == null) {
				ErrorHelper.Warning (137, Errors.mtouch.MT0137, ar.FullName, main.Name, ca.AttributeType.FullName);
				return;
			}
			ComputeListOfAssemblies (assemblies, reference_assembly, exceptions);
		}
		...
		public void ManagedLink ()
		{
			...
			foreach (var group in output_assemblies.GroupBy ((v) => v.Name.Name)) {
				if (group.Count () != 1)
					throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_N, String.Format(Errors.mtouch.MT0175_a, group.Key, string.Join("\n\t", group.Select((v) => v.MainModule.FileName).ToArray())));
			}
			...
			if (sharingTargets.Count == 0) {
				...
				foreach (var t in allTargets) {
					...
					do {
						...
						if (ad == null)
							throw ErrorHelper.CreateError(99, Errors.mtouch.MT0099_N, String.Format(Errors.mtouch.MT0174_a, next));
		...
		public void CompilePInvokeWrappers ()
		{
			...
			foreach (var abi in GetArchitectures (mode)) {
				...
				case AssemblyBuildTarget.Framework:
					...
					break;
				default:
					throw ErrorHelper.CreateError (100, Errors.mtouch.MT0100, mode);
				}
				...
				foreach (var abi in GetArchitectures (build_target)) {
					...
					if (existingLinkTask.Count > 0) {
						if (existingLinkTask.Count != infos.Count)
							throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_N, String.Format(Errors.mtouch.MT0175_b, name));
						if (!existingLinkTask.All ((v) => v == existingLinkTask [0]))
							throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_N, String.Format(Errors.mtouch.MT0175_c, name));
					...
					foreach (var a in assemblies) {
						...
						if (a.HasLinkWithAttributes) {
							...
							case SymbolMode.Linker:
								...
								break;
							default:
								throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099, App.SymbolMode);
							}
						}
					}
					if (App.Embeddinator)
					...
					if (App.EnableLLVMOnlyBitCode) {
						...
						if (optimizations.Count == 0) {
							...
							throw ErrorHelper.CreateError (107, Errors.mtouch.MT0107, string.Join (", ", assemblies.Select ((v) => v.Identity)), string.Join ("', '", optimizations));
						}
					}
		...
		public void Compile ()
		{
			...
			if (exceptions.Count > 0) {
				...
				ErrorHelper.Warning (3006, Errors.mtouch.MT3006);
			}
		...
		public NativeLinkTask NativeLink (BuildTasks build_tasks, Abi abi, string output_file)
		{
			...
			case SymbolMode.Linker:
				...
				break;
			default:
				throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_O, App.SymbolMode);
			}
			...
			if (App.EnableProfiling) {
				...
				default:
					throw ErrorHelper.CreateError (100, Errors.mtouch.MT0100, App.LibProfilerLinkMode);
				}
			}
					...
					while ((line = reader.ReadLine ()) != null) {
						...
						switch (typestr) {
							case "RequireMonoNative":
								...
								break;
							default:
								throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_P, typestr);
						}
					}
				}
			}
		...
		void HandleMonoNative (Application app, CompilerFlags compiler_flags)
		{
			...
			case AssemblyBuildTarget.StaticObject:
				...
				break;
			default:
				throw ErrorHelper.CreateError (100, Errors.mtouch.MT0100, app.LibMonoLinkMode);
			}
		}
		...
		public void Symlink ()
		{
			...
			if (Abis.Count != 1)
				throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_M, string.Join (", ", Abis.Select ((v) => v.ToString ())));
			...
			try {
				...
				throw new MonoTouchException (1015, true, ex, Errors.mtouch.MT1015, targetExecutable, ex.Message);
			}

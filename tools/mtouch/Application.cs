...
namespace Xamarin.Bundler {
	...
	public partial class Application
	{
		...
		public void AddAssemblyBuildTarget (string value)
		{
			...
			if (eq_index == -1)
				throw ErrorHelper.CreateError (10, Xamarin.Bundler.Errors.MT0010, $"--assembly-build-target={value}");
			...
			if ((invalid_idx = name.IndexOfAny (new char [] { '/', '\\' })) != -1)
				throw ErrorHelper.CreateError (106, Xamarin.Bundler.Errors.MT0106, name, name [invalid_idx]);

			if (assembly_build_targets.ContainsKey (assembly_name))
				throw ErrorHelper.CreateError (101, Xamarin.Bundler.Errors.MT0101, assembly_name);
			...
			if (IsSimulatorBuild && !Embeddinator) {
				if (assembly_build_targets.Count > 0) {
					...
					if (assembly_build_targets.Count == 1 && first.Key == "@all" && first.Value.Item1 == AssemblyBuildTarget.DynamicLibrary && first.Value.Item2 == string.Empty) {
						ErrorHelper.Warning (126, Xamarin.Bundler.Errors.MT0126);
					} else {
						ErrorHelper.Warning (125, Xamarin.Bundler.Errors.MT0125);
					}
					assembly_build_targets.Clear ();
				}
				return;
			...
			foreach (var target in Targets) {
				...
				foreach (var assembly in target.Assemblies) {
					...
					if (build_target == null) {
						...
						exceptions.Add (ErrorHelper.CreateError (105, Xamarin.Bundler.Errors.MT0105, assembly.Identity));
						continue;
					}
				...
				foreach (var abt in asm_build_targets) {
					...
					if (exceptions == null)
						exceptions = new List<Exception> ();
					exceptions.Add (ErrorHelper.CreateError (108, Xamarin.Bundler.Errors.MT0108,  abt.Key));
				}
				...
				foreach (var @group in grouped) {
					...
					for (int i = 1; i < assemblies.Length; i++) {
						if (assemblies [0].BuildTarget != assemblies [i].BuildTarget)
							throw ErrorHelper.CreateError (102, Xamarin.Bundler.Errors.MT0102,
					...
					if (assemblies.Length != 1 && assemblies [0].BuildTarget == AssemblyBuildTarget.StaticObject)
						throw ErrorHelper.CreateError (103, Xamarin.Bundler.Errors.MT0103,
		...
		public bool UseDlsym (string assembly)
		{
			...
			case ApplePlatform.WatchOS:
				return false;
			default:
				throw ErrorHelper.CreateError (71, Xamarin.Bundler.Errors.MT0071, Platform);
			}
		}
		...
		public void SetDefaultAbi ()
		{
			...
			case ApplePlatform.WatchOS:
				if (abis.Count == 0)
					throw ErrorHelper.CreateError (76, Xamarin.Bundler.Errors.MT0076, "Xamarin.WatchOS");
				break;
			case ApplePlatform.TVOS:
				if (abis.Count == 0)
					throw ErrorHelper.CreateError (76, Xamarin.Bundler.Errors.MT0076, "Xamarin.TVOS");
				break;
			default:
				throw ErrorHelper.CreateError (71, Xamarin.Bundler.Errors.MT0071, Platform);
			}
		...
		public void ValidateAbi ()
		{
			...
			foreach (var abi in abis) {
				if (!validAbis.Contains (abi))
					throw ErrorHelper.CreateError (75, Xamarin.Bundler.Errors.MT0075, abi, Platform, string.Join (", ", validAbis.Select ((v) => v.AsString ()).ToArray ()));
			}
		}
		...
		public void ParseAbi (string abi)
		{
			...
			foreach (var str in abi.Split (new char [] { ',' }, StringSplitOptions.RemoveEmptyEntries)) {
				...
				case "armv7k+llvm":
					...
					break;
				default:
					throw new MonoTouchException (15, true, Xamarin.Bundler.Errors.MT0015, str);
				}
			...
			if (!Driver.HasTargetFramework) {
				foreach (var reference in References) {
					...
					case "Xamarin.WatchOS.dll":
						throw ErrorHelper.CreateError (86, Xamarin.Bundler.Errors.MT0086);
					}
		...
		void DetectCodeSharing ()
		{
			...
			if (Platform == ApplePlatform.iOS && DeploymentTarget.Major < 8) {
				...
				ErrorHelper.Warning (112, Xamarin.Bundler.Errors.MT0112, String.Format (Xamarin.Bundler.Errors.MT0112_a, DeploymentTarget));
				return;
			}
			...
			if (I18n != Mono.Linker.I18nAssemblies.None) {
				...
				ErrorHelper.Warning (112, Xamarin.Bundler.Errors.MT0112, String.Format(Xamarin.Bundler.Errors.MT0112_b, I18n));
				return;
			}
			...
			foreach (var appex in AppExtensions) {
				...
				if (BitCodeMode != appex.BitCodeMode) {
					ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_a, appex.BitCodeMode, BitCodeMode));
					continue;
				}
				...
				if (assembly_build_targets.Count != appex.assembly_build_targets.Count) {
					ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113,appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_b, FormatAssemblyBuildTargets(), appex.FormatAssemblyBuildTargets()));
					continue;
				}
				...
				if (appex.I18n != Mono.Linker.I18nAssemblies.None) {
					ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_c, I18n, appex.I18n));
					continue;
				}

				// All arguments to the AOT compiler must be identical
				if (AotArguments != appex.AotArguments) {
					ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_d, AotArguments, appex.AotArguments));
					continue;
				}

				if (!CompareLists (AotOtherArguments, appex.AotOtherArguments)) {
					ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_e, StringUtils.FormatArguments(AotOtherArguments), StringUtils.FormatArguments(appex.AotOtherArguments)));
					continue;
				}

				if (IsLLVM != appex.IsLLVM) {
					ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_f, IsLLVM, appex.IsLLVM));
				}

				if (LinkMode != appex.LinkMode) {
					ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_g, LinkMode, appex.LinkMode));
					continue;
				}
				...
				if (LinkMode != LinkMode.None) {
					...
					if (!linkskipped_same) {
						ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_h, string.Join(", ", LinkSkipped), string.Join(", ", appex.LinkSkipped)));
						continue;
					}

					if (Definitions.Count > 0) {
						ErrorHelper.Warning (112, Xamarin.Bundler.Errors.MT0112, String.Format(Xamarin.Bundler.Errors.MT0112_c, string.Join(", ", Definitions)));
						continue;
					}

					if (appex.Definitions.Count > 0) {
						ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_i, string.Join(", ", appex.Definitions)));
						continue;
					}
				}

				if (UseInterpreter != appex.UseInterpreter) {
					ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_j, (UseInterpreter ? "Enabled" : "Disabled"), (appex.UseInterpreter ? "Enabled" : "Disabled")));
					continue;
					...
					if (!appAssemblies.SetEquals (appexAssemblies)) {
						ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_k, (InterpretedAssemblies.Count == 0 ? "all assemblies" : string.Join(", ", InterpretedAssemblies)), (appex.InterpretedAssemblies.Count == 0 ? "all assemblies" : string.Join(", ", appex.InterpretedAssemblies))));
						continue;
					}
				}
				...
				foreach (var abi in appex.Abis) {
					...
					if (matching == Abi.None) {
						...
						ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_l, abi));
						applicable = false;
						break;
					} else if (matching != abi) {
						// Example: extension has arm64+llvm, while the main app has only arm64.
						ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_m, matching, abi));
						applicable = false;
						break;
					}
				}
				...
				if (Optimizations.RemoveDynamicRegistrar != appex.Optimizations.RemoveDynamicRegistrar) {
					...
					ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_n, bool_tostr(appex.Optimizations.RemoveDynamicRegistrar), bool_tostr(Optimizations.RemoveDynamicRegistrar)));
					continue;
				}
				...
				foreach (var target in Targets) {
					...
					foreach (var kvp in appexTarget.Assemblies.Hashed) {
						...
						if (asm.FullPath != kvp.Value.FullPath && !Cache.CompareFiles (asm.FullPath, kvp.Value.FullPath, true)) {
							...
							ErrorHelper.Warning (113, Xamarin.Bundler.Errors.MT0113, appex.Name, String.Format(Xamarin.Bundler.Errors.MT0113_n, asm.Identity, asm.FullPath));
							break;
						}
					}
				}
		...
		void Initialize ()
		{
			if (EnableDebug && IsLLVM)
				ErrorHelper.Warning (3003, Xamarin.Bundler.Errors.MT3003);

			if (!IsLLVM && (EnableAsmOnlyBitCode || EnableLLVMOnlyBitCode))
				throw ErrorHelper.CreateError (3008, Xamarin.Bundler.Errors.MT3008);

			if (IsLLVM && Platform == ApplePlatform.WatchOS && BitCodeMode != BitCodeMode.LLVMOnly) {
				ErrorHelper.Warning (111, Xamarin.Bundler.Errors.MT0111);
			...
			if (!DebugTrack.HasValue) {
				...
				ErrorHelper.Warning (32, Xamarin.Bundler.Errors.MT0032);
			}
			...
			foreach (var root in RootAssemblies) {
				...
				if (root [0] == '-' || root [0] == '/') {
					exceptions.Add (ErrorHelper.CreateError (18, Xamarin.Bundler.Errors.MT0018, root));
				} else {
					exceptions.Add (ErrorHelper.CreateError (7, Xamarin.Bundler.Errors.MT0007, root));
				}
			}
			...
			if (no_framework)
				throw ErrorHelper.CreateError (96, Xamarin.Bundler.Errors.MT0096);
			...
			foreach (var reference in References) {
				...
				if (name == Driver.GetProductAssembly (this)) {
					...
					case "Xamarin.WatchOS":
						throw ErrorHelper.CreateError (41, Xamarin.Bundler.Errors.MT0041, Path.GetFileName (reference), Driver.TargetFramework.Identifier);
					}
				}
			}
			if (!platformAssemblyReference) {
				ErrorHelper.Warning (85, Xamarin.Bundler.Errors.MT0085, Driver.GetProductAssembly (this) + ".dll");
			...
			foreach (var root in RootAssemblies) {
				...
				if (Profile.IsSdkAssembly (root_wo_ext) || Profile.IsProductAssembly (root_wo_ext))
					throw new MonoTouchException (3, true, Xamarin.Bundler.Errors.MT0003, root_wo_ext);
			}
			...
			if (ExecutableName != Path.GetFileNameWithoutExtension (AppDirectory))
				ErrorHelper.Warning (30, Xamarin.Bundler.Errors.MT0030,
					ExecutableName, Path.GetFileName (AppDirectory));
			
			if (IsExtension && Platform == ApplePlatform.iOS && SdkVersion < new Version (8, 0))
				throw new MonoTouchException (45, true, Xamarin.Bundler.Errors.MT0045);

			if (IsExtension && Platform != ApplePlatform.iOS && Platform != ApplePlatform.WatchOS && Platform != ApplePlatform.TVOS)
				throw new MonoTouchException (72, true, Xamarin.Bundler.Errors.MT0072, Platform);

			if (!IsExtension && Platform == ApplePlatform.WatchOS)
				throw new MonoTouchException (77, true, Xamarin.Bundler.Errors.MT0077);
			...
			if (Platform == ApplePlatform.iOS && (HasDynamicLibraries || HasFrameworks) && DeploymentTarget.Major < 8) {
				ErrorHelper.Warning (78, Xamarin.Bundler.Errors.MT0078, DeploymentTarget);
			...
			if (!package_managed_debug_symbols.HasValue) {
				...
				ErrorHelper.Warning (3007, Xamarin.Bundler.Errors.MT3007);
			}
			...
			if (Frameworks.Count > 0) {
				...
				case ApplePlatform.iOS:
					if (DeploymentTarget < new Version (8, 0))
						throw ErrorHelper.CreateError (65, Xamarin.Bundler.Errors.MT0065, DeploymentTarget, string.Join (", ", Frameworks.ToArray ()));
					break;
				case ApplePlatform.WatchOS:
					if (DeploymentTarget < new Version (2, 0))
						throw ErrorHelper.CreateError (65, Xamarin.Bundler.Errors.MT0065_A, DeploymentTarget, string.Join (", ", Frameworks.ToArray ()));
					break;
				case ApplePlatform.TVOS:
			...
			if (IsDeviceBuild) {
				...
				case BitCodeMode.ASMOnly:
					if (Platform == ApplePlatform.WatchOS)
						throw ErrorHelper.CreateError (83, Xamarin.Bundler.Errors.MT0083);
					break;
			...
			if (EnableBitCode && IsSimulatorBuild)
				throw ErrorHelper.CreateError (84, Xamarin.Bundler.Errors.MT0084);
			...
			if (Platform == ApplePlatform.iOS) {
				if (DeploymentTarget.Major >= 11 && Is32Build) {
					...
					throw ErrorHelper.CreateError (116, Xamarin.Bundler.Errors.MT0116, invalidArches.First ());
				}
			}
		...
		void WriteNotice (string directory)
		{
			...
			if (Directory.Exists (path))
				throw new MonoTouchException (1016, true, Xamarin.Bundler.Errors.MT1016);
			...
			try {
				...
				throw new MonoTouchException (1017, true, ex, Xamarin.Bundler.Errors.MT1017, ex.Message);
			}
		}
			...
			try {
				if (p.Start ()) {
					...
					if (p.ExitCode == 0)
						...
						ErrorHelper.Warning (95, Xamarin.Bundler.Errors.MT0095, dest, error); 
						return;
					}
				}

				ErrorHelper.Warning (95, Xamarin.Bundler.Errors.MT0095_A, dest);
				return;
			}
			catch (Exception e) {
				ErrorHelper.Warning (95, e, Xamarin.Bundler.Errors.MT0095_A, dest);
				return;
			}
		}
		...
		void BuildBundle ()
		{
			...
			foreach (var kvp in bundle_files) {
				...
				if (!files.All ((v) => Directory.Exists (v) == isFramework))
					throw ErrorHelper.CreateError (99, Xamarin.Bundler.Errors.MT0099, $"'can't process a mix of dylibs and frameworks: {string.Join(", ", files)}'");
				...
				if (isFramework) {
					...
					if (files.Count != 1) {
						...
						exceptions.Add (ErrorHelper.CreateError (1035, Xamarin.Bundler.Errors.MT1035, fname));
						foreach (var file in files)
							exceptions.Add (ErrorHelper.CreateError (1036, Xamarin.Bundler.Errors.MT1036, fname, file));
						throw new AggregateException (exceptions);
					}
					if (info.DylibToFramework)
						throw ErrorHelper.CreateError (99, Xamarin.Bundler.Errors.MT0099, $"'can't convert frameworks to frameworks: {files.First()}'");
					...
					try {
						...
						throw ErrorHelper.CreateError (140, e, Xamarin.Bundler.Errors.MT0140, framework_filename);
					}
					if (!dynamic) {
				...
				if (line.Contains ("Undefined symbols for architecture")) {
					while (++i < lines.Count) {
						...
						if (symbol.StartsWith ("_OBJC_CLASS_$_", StringComparison.Ordinal)) {
							errors.Add (new MonoTouchException (5211, error, Xamarin.Bundler.Errors.MT5211,
							...
							if (members != null && members.Any ()) {
								...
								errors.Add (new MonoTouchException (5214, error, Xamarin.Bundler.Errors.MT5214,
									symbol, member.DeclaringType.FullName, member.Name));
							} else {
								errors.Add (new MonoTouchException (5210, error, Xamarin.Bundler.Errors.MT5210,
																symbol));
							}
						}
					...
					errors.Add (new MonoTouchException (5212, error, Xamarin.Bundler.Errors.MT5212, symbol));
					...
					while (i + 1 < lines.Count) {
						...
						if (GetIndentation (lines [i + 1]) <= indent)
							break;
						i++;
						errors.Add (new MonoTouchException (5213, error, Xamarin.Bundler.Errors.MT5213, line.Trim ()));
					}
				} else {
					...
					if (error) {
						errors.Add (new MonoTouchException (5209, error, Xamarin.Bundler.Errors.MT5209, line));
					} else {
						errors.Add (new MonoTouchException (5203, error, Xamarin.Bundler.Errors.MT5203, line));
					}
				}
			}
		}
		...
		{
			if (string.IsNullOrEmpty (src) || string.IsNullOrEmpty (dest)) {
				ErrorHelper.Warning (95, Xamarin.Bundler.Errors.MT0095_B, dest);
				return;
			}
		...
		public void BuildMSymDirectory ()
		{
			...
			foreach (var target in Targets) {
				...
				if (!Directory.Exists (msymdir)) {
					ErrorHelper.Warning (118, Xamarin.Bundler.Errors.MT0118, msymdir);
					continue;
				}
				...
				if (IsExtension && !IsWatchExtension) {
					...
					if (codeShared > 0) {
						if (codeShared != assemblies.Length)
							throw ErrorHelper.CreateError (99, Xamarin.Bundler.Errors.MT0099, $"all assemblies in a joined build target must have the same code sharing options ({string.Join(", ", assemblies.Select((v) => v.Identity + "=" + v.IsCodeShared))})");

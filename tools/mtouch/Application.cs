...
namespace Xamarin.Bundler {
	...
	public partial class Application
	{
		...
		public void AddAssemblyBuildTarget (string value)
		{
			...
			if (eq_index == -1)
				throw ErrorHelper.CreateError (10, Errors.mtouch.MT0010, $"--assembly-build-target={value}");
			...
			if ((invalid_idx = name.IndexOfAny (new char [] { '/', '\\' })) != -1)
				throw ErrorHelper.CreateError (106, Errors.mtouch.MT0106, name, name [invalid_idx]);

			if (assembly_build_targets.ContainsKey (assembly_name))
				throw ErrorHelper.CreateError (101, Errors.mtouch.MT0101, assembly_name);
			...
			if (IsSimulatorBuild && !Embeddinator) {
				if (assembly_build_targets.Count > 0) {
					...
					if (assembly_build_targets.Count == 1 && first.Key == "@all" && first.Value.Item1 == AssemblyBuildTarget.DynamicLibrary && first.Value.Item2 == string.Empty) {
						ErrorHelper.Warning (126, Errors.mtouch.MT0126);
					} else {
						ErrorHelper.Warning (125, Errors.mtouch.MT0125);
					}
					assembly_build_targets.Clear ();
				}
				return;
			...
			foreach (var target in Targets) {
				...
				foreach (var assembly in target.Assemblies) {
					...
					if (build_target == null) {
						...
						exceptions.Add (ErrorHelper.CreateError (105, Errors.mtouch.MT0105, assembly.Identity));
						continue;
					}
				...
				foreach (var abt in asm_build_targets) {
					...
					if (exceptions == null)
						exceptions = new List<Exception> ();
					exceptions.Add (ErrorHelper.CreateError (108, Errors.mtouch.MT0108,  abt.Key));
				}
				...
				foreach (var @group in grouped) {
					...
					for (int i = 1; i < assemblies.Length; i++) {
						if (assemblies [0].BuildTarget != assemblies [i].BuildTarget)
							throw ErrorHelper.CreateError (102, Errors.mtouch.MT0102,
					...
					if (assemblies.Length != 1 && assemblies [0].BuildTarget == AssemblyBuildTarget.StaticObject)
						throw ErrorHelper.CreateError (103, Errors.mtouch.MT0103,
		...
		public bool UseDlsym (string assembly)
		{
			...
			case ApplePlatform.WatchOS:
				return false;
			default:
				throw ErrorHelper.CreateError (71, Errors.mtouch.MT0071, Platform);
			}
		}
		...
		public void SetDefaultAbi ()
		{
			...
			case ApplePlatform.WatchOS:
				if (abis.Count == 0)
					throw ErrorHelper.CreateError (76, Errors.mtouch.MT0076, "Xamarin.WatchOS");
				break;
			case ApplePlatform.TVOS:
				if (abis.Count == 0)
					throw ErrorHelper.CreateError (76, Errors.mtouch.MT0076, "Xamarin.TVOS");
				break;
			default:
				throw ErrorHelper.CreateError (71, Errors.mtouch.MT0071, Platform);
			}
		...
		public void ValidateAbi ()
		{
			...
			foreach (var abi in abis) {
				if (!validAbis.Contains (abi))
					throw ErrorHelper.CreateError (75, Errors.mtouch.MT0075, abi, Platform, string.Join (", ", validAbis.Select ((v) => v.AsString ()).ToArray ()));
			}
		}
		...
		public void ParseAbi (string abi)
		{
			...
			foreach (var str in abi.Split (new char [] { ',' }, StringSplitOptions.RemoveEmptyEntries)) {
				...
				case "armv7k+llvm":
					...
					break;
				default:
					throw new MonoTouchException (15, true, Errors.mtouch.MT0015, str);
				}
			...
			if (!Driver.HasTargetFramework) {
				foreach (var reference in References) {
					...
					case "Xamarin.WatchOS.dll":
						throw ErrorHelper.CreateError (86, Errors.mtouch.MT0086);
					}
		...
		void DetectCodeSharing ()
		{
			...
			if (Platform == ApplePlatform.iOS && DeploymentTarget.Major < 8) {
				...
				ErrorHelper.Warning (112, Errors.mtouch.MT0112, String.Format (Errors.mtouch.MT0112_a, DeploymentTarget));
				return;
			}
			...
			if (I18n != Mono.Linker.I18nAssemblies.None) {
				...
				ErrorHelper.Warning (112, Errors.mtouch.MT0112, String.Format(Errors.mtouch.MT0112_b, I18n));
				return;
			}
			...
			foreach (var appex in AppExtensions) {
				...
				if (BitCodeMode != appex.BitCodeMode) {
					ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_a, appex.BitCodeMode, BitCodeMode));
					continue;
				}
				...
				if (assembly_build_targets.Count != appex.assembly_build_targets.Count) {
					ErrorHelper.Warning (113, Errors.mtouch.MT0113,appex.Name, String.Format(Errors.mtouch.MT0113_b, FormatAssemblyBuildTargets(), appex.FormatAssemblyBuildTargets()));
					continue;
				}
				...
				if (appex.I18n != Mono.Linker.I18nAssemblies.None) {
					ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_c, I18n, appex.I18n));
					continue;
				}

				// All arguments to the AOT compiler must be identical
				if (AotArguments != appex.AotArguments) {
					ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_d, AotArguments, appex.AotArguments));
					continue;
				}

				if (!CompareLists (AotOtherArguments, appex.AotOtherArguments)) {
					ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_e, StringUtils.FormatArguments(AotOtherArguments), StringUtils.FormatArguments(appex.AotOtherArguments)));
					continue;
				}

				if (IsLLVM != appex.IsLLVM) {
					ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_f, IsLLVM, appex.IsLLVM));
				}

				if (LinkMode != appex.LinkMode) {
					ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_g, LinkMode, appex.LinkMode));
					continue;
				}
				...
				if (LinkMode != LinkMode.None) {
					...
					if (!linkskipped_same) {
						ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_h, string.Join(", ", LinkSkipped), string.Join(", ", appex.LinkSkipped)));
						continue;
					}

					if (Definitions.Count > 0) {
						ErrorHelper.Warning (112, Errors.mtouch.MT0112, String.Format(Errors.mtouch.MT0112_c, string.Join(", ", Definitions)));
						continue;
					}

					if (appex.Definitions.Count > 0) {
						ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_i, string.Join(", ", appex.Definitions)));
						continue;
					}
				}

				if (UseInterpreter != appex.UseInterpreter) {
					ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_j, (UseInterpreter ? "Enabled" : "Disabled"), (appex.UseInterpreter ? "Enabled" : "Disabled")));
					continue;
					...
					if (!appAssemblies.SetEquals (appexAssemblies)) {
						ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_k, (InterpretedAssemblies.Count == 0 ? "all assemblies" : string.Join(", ", InterpretedAssemblies)), (appex.InterpretedAssemblies.Count == 0 ? "all assemblies" : string.Join(", ", appex.InterpretedAssemblies))));
						continue;
					}
				}
				...
				foreach (var abi in appex.Abis) {
					...
					if (matching == Abi.None) {
						...
						ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_l, abi));
						applicable = false;
						break;
					} else if (matching != abi) {
						// Example: extension has arm64+llvm, while the main app has only arm64.
						ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_m, matching, abi));
						applicable = false;
						break;
					}
				}
				...
				if (Optimizations.RemoveDynamicRegistrar != appex.Optimizations.RemoveDynamicRegistrar) {
					...
					ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_n, bool_tostr(appex.Optimizations.RemoveDynamicRegistrar), bool_tostr(Optimizations.RemoveDynamicRegistrar)));
					continue;
				}
				...
				foreach (var target in Targets) {
					...
					foreach (var kvp in appexTarget.Assemblies.Hashed) {
						...
						if (asm.FullPath != kvp.Value.FullPath && !Cache.CompareFiles (asm.FullPath, kvp.Value.FullPath, true)) {
							...
							ErrorHelper.Warning (113, Errors.mtouch.MT0113, appex.Name, String.Format(Errors.mtouch.MT0113_n, asm.Identity, asm.FullPath));
							break;
						}
					}
				}
		...
		void Initialize ()
		{
			if (EnableDebug && IsLLVM)
				ErrorHelper.Warning (3003, Errors.mtouch.MT3003);

			if (!IsLLVM && (EnableAsmOnlyBitCode || EnableLLVMOnlyBitCode))
				throw ErrorHelper.CreateError (3008, Errors.mtouch.MT3008);

			if (IsLLVM && Platform == ApplePlatform.WatchOS && BitCodeMode != BitCodeMode.LLVMOnly) {
				ErrorHelper.Warning (111, Errors.mtouch.MT0111);
			...
			if (!DebugTrack.HasValue) {
				...
				ErrorHelper.Warning (32, Errors.mtouch.MT0032);
			}
			...
			foreach (var root in RootAssemblies) {
				...
				if (root [0] == '-' || root [0] == '/') {
					exceptions.Add (ErrorHelper.CreateError (18, Errors.mtouch.MT0018, root));
				} else {
					exceptions.Add (ErrorHelper.CreateError (7, Errors.mtouch.MT0007, root));
				}
			}
			...
			if (no_framework)
				throw ErrorHelper.CreateError (96, Errors.mtouch.MT0096);
			...
			foreach (var reference in References) {
				...
				if (name == Driver.GetProductAssembly (this)) {
					...
					case "Xamarin.WatchOS":
						throw ErrorHelper.CreateError (41, Errors.mtouch.MT0041, Path.GetFileName (reference), Driver.TargetFramework.Identifier);
					}
				}
			}
			if (!platformAssemblyReference) {
				ErrorHelper.Warning (85, Errors.mtouch.MT0085, Driver.GetProductAssembly (this) + ".dll");
			...
			foreach (var root in RootAssemblies) {
				...
				if (Profile.IsSdkAssembly (root_wo_ext) || Profile.IsProductAssembly (root_wo_ext))
					throw new MonoTouchException (3, true, Errors.mtouch.MT0003, root_wo_ext);
			}
			...
			if (Platform == ApplePlatform.iOS && (HasDynamicLibraries || HasFrameworks) && DeploymentTarget.Major < 8) {
				ErrorHelper.Warning (78, Errors.mtouch.MT0078, DeploymentTarget);
			...
			if (!package_managed_debug_symbols.HasValue) {
				...
				ErrorHelper.Warning (3007, Errors.mtouch.MT3007);
			}
			...
			if (Frameworks.Count > 0) {
				...
				case ApplePlatform.iOS:
					if (DeploymentTarget < new Version (8, 0))
						throw ErrorHelper.CreateError (65, Errors.mtouch.MT0065, DeploymentTarget, string.Join (", ", Frameworks.ToArray ()));
					break;
				case ApplePlatform.WatchOS:
					if (DeploymentTarget < new Version (2, 0))
						throw ErrorHelper.CreateError (65, Errors.mtouch.MT0065_A, DeploymentTarget, string.Join (", ", Frameworks.ToArray ()));
					break;
				case ApplePlatform.TVOS:
			...
			if (IsDeviceBuild) {
				...
				case BitCodeMode.ASMOnly:
					if (Platform == ApplePlatform.WatchOS)
						throw ErrorHelper.CreateError (83, Errors.mtouch.MT0083);
					break;
			...
			if (EnableBitCode && IsSimulatorBuild)
				throw ErrorHelper.CreateError (84, Errors.mtouch.MT0084);
			...
			if (Platform == ApplePlatform.iOS) {
				if (DeploymentTarget.Major >= 11 && Is32Build) {
					...
					throw ErrorHelper.CreateError (116, Errors.mtouch.MT0116, invalidArches.First ());
				}
			}
		...
		void WriteNotice (string directory)
		{
			...
			if (Directory.Exists (path))
				throw new MonoTouchException (1016, true, Errors.mtouch.MT1016);
			...
			try {
				...
				throw new MonoTouchException (1017, true, ex, Errors.mtouch.MT1017, ex.Message);
			}
		}
			...
			try {
				if (p.Start ()) {
					...
					if (p.ExitCode == 0)
						...
						ErrorHelper.Warning (95, Errors.mtouch.MT0095, dest, error); 
						return;
					}
				}

				ErrorHelper.Warning (95, Errors.mtouch.MT0095_A, dest);
				return;
			}
			catch (Exception e) {
				ErrorHelper.Warning (95, e, Errors.mtouch.MT0095_A, dest);
				return;
			}
		}
		...
		void BuildBundle ()
		{
			...
			foreach (var kvp in bundle_files) {
				...
				if (!files.All ((v) => Directory.Exists (v) == isFramework))
					throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_I, string.Join (", ", files));
				...
				if (isFramework) {
					...
					if (files.Count != 1) {
						...
						exceptions.Add (ErrorHelper.CreateError (1035, Errors.mtouch.MT1035, fname));
						foreach (var file in files)
							exceptions.Add (ErrorHelper.CreateError (1036, Errors.mtouch.MT1036, fname, file));
						throw new AggregateException (exceptions);
					}
					if (info.DylibToFramework)
						throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099, files.First ());
					...
					try {
						...
						throw ErrorHelper.CreateError (140, e, Errors.mtouch.MT0140, framework_filename);
					}
					if (!dynamic) {
		...
		public string GetLibMono (AssemblyBuildTarget build_target)
		{
			...
			default:
				throw ErrorHelper.CreateError (100, Errors.mtouch.MT0100, build_target);
			}
		}
				...
				if (line.Contains ("Undefined symbols for architecture")) {
					while (++i < lines.Count) {
						...
						if (symbol.StartsWith ("_OBJC_CLASS_$_", StringComparison.Ordinal)) {
							errors.Add (new MonoTouchException (5211, error, Errors.mtouch.MT5211,
							...
							if (members != null && members.Any ()) {
								...
								errors.Add (new MonoTouchException (5214, error, Errors.mtouch.MT5214,
									symbol, member.DeclaringType.FullName, member.Name));
							} else {
								errors.Add (new MonoTouchException (5210, error, Errors.mtouch.MT5210,
																symbol));
							}
						}
					...
					errors.Add (new MonoTouchException (5212, error, Errors.mtouch.MT5212, symbol));
					...
					while (i + 1 < lines.Count) {
						...
						if (GetIndentation (lines [i + 1]) <= indent)
							break;
						i++;
						errors.Add (new MonoTouchException (5213, error, Errors.mtouch.MT5213, line.Trim ()));
					}
				} else {
					...
					if (error) {
						errors.Add (new MonoTouchException (5209, error, Errors.mtouch.MT5209, line));
					} else {
						errors.Add (new MonoTouchException (5203, error, Errors.mtouch.MT5203, line));
					}
				}
			}
		}
		...
		{
			if (string.IsNullOrEmpty (src) || string.IsNullOrEmpty (dest)) {
				ErrorHelper.Warning (95, Errors.mtouch.MT0095_B, dest);
				return;
			}
		...
		public void BuildMSymDirectory ()
		{
			...
			foreach (var target in Targets) {
				...
				if (!Directory.Exists (msymdir)) {
					ErrorHelper.Warning (118, Errors.mtouch.MT0118, msymdir);
					continue;
				}
				...
				if (IsExtension && !IsWatchExtension) {
					...
					if (codeShared > 0) {
						if (codeShared != assemblies.Length)
							throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_J, string.Join (", ", assemblies.Select ((v) => v.Identity + "=" + v.IsCodeShared)));

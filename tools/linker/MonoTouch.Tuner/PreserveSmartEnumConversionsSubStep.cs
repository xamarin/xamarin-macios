		...
		void ProcessAttributeProvider (ICustomAttributeProvider provider, MethodDefinition conditionA, MethodDefinition conditionB = null)
		{
			...
			foreach (var ca in provider.CustomAttributes) {
				...
				if (ca.ConstructorArguments.Count != 1) {
					ErrorHelper.Show (ErrorHelper.CreateWarning (LinkContext.Target.App, 4124, provider, Errors.mtouch.MT4124_E, provider.AsString (), ca.ConstructorArguments.Count));
					continue;
				}
				...
				if (managedEnumType == null) {
					ErrorHelper.Show (ErrorHelper.CreateWarning (LinkContext.Target.App, 4124, provider, Errors.mtouch.MT4124_H, provider.AsString (), managedType?.FullName));
					continue;
				}

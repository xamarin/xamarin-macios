		...
		void ProcessAttributeProvider (ICustomAttributeProvider provider, MethodDefinition conditionA, MethodDefinition conditionB = null)
		{
			...
			foreach (var ca in provider.CustomAttributes) {
				...
				if (ca.ConstructorArguments.Count != 1) {
					ErrorHelper.Show (ErrorHelper.CreateWarning (LinkContext.Target.App, 4190, provider, mtouch.Errors.MT4190, provider.AsString (), ca.ConstructorArguments.Count));
					continue;
				}
				...
				if (managedEnumType == null) {
					ErrorHelper.Show (ErrorHelper.CreateWarning (LinkContext.Target.App, 4191, provider, mtouch.Errors.MT4191, provider.AsString (), managedType?.FullName));
					continue;
				}

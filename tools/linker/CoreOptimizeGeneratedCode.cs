	...
	public abstract class CoreOptimizeGeneratedCode : ExceptionalSubStep {
			...
			if (Optimizations.InlineIsARM64CallingConvention == true) { 
				if (options.Target.Abis.Count == 1) {
					...
					case Abi.ARMv7k:
						...
						break;
					default:
						options.LinkContext.Exceptions.Add (ErrorHelper.CreateWarning (178, mtouch.mtouchErrors.MT0178, options.Target.Abis [0]));
						break;
					}
		...
		MethodReference GetBlockSetupImpl (MethodDefinition caller, Instruction ins)
		{
			if (setupblock_def == null) {
				...
				if (setupblock_def == null)
					throw ErrorHelper.CreateError (Options.Application, 179, caller, ins, mtouch.mtouchErrors.MT0179,  Namespaces.ObjCRuntime);
			}

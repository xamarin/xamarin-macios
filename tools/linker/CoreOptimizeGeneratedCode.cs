	...
	public abstract class CoreOptimizeGeneratedCode : ExceptionalSubStep {
			...
			if (Optimizations.InlineIsARM64CallingConvention == true) { 
				if (options.Target.Abis.Count == 1) {
					...
					case Abi.ARMv7k:
						...
						break;
					default:
						options.LinkContext.Exceptions.Add (ErrorHelper.CreateWarning (99, Errors.mtouch.MT0099_G, options.Target.Abis [0]));
						break;
					}
		...
		MethodReference GetBlockSetupImpl (MethodDefinition caller, Instruction ins)
		{
			if (setupblock_def == null) {
				...
				if (setupblock_def == null)
					throw ErrorHelper.CreateError (Options.Application, 99, caller, ins, Errors.mtouch.MT0099_H,  Namespaces.ObjCRuntime);
			}

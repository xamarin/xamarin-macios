	...
	public static class MobileExtensions {
		...
		static BindingImplOptions? GetBindingImplAttribute (ICustomAttributeProvider provider, IEnumerable<ICustomAttribute> attributes)
		{
			...
			foreach (var ca in attributes) {
				...
				if (ca.HasFields)
					throw ErrorHelper.CreateError (2105, Xamarin.Bundler.Errors.MT2105_A, provider.AsString ());
				if (ca.HasProperties)
					throw ErrorHelper.CreateError (2105, Xamarin.Bundler.Errors.MT2105_B, provider.AsString ());

				switch (ca.ConstructorArguments.Count) {
				case 1:
					var arg = ca.ConstructorArguments [0];
					if (!arg.Type.Is (Namespaces.ObjCRuntime, "BindingImplOptions"))
						throw ErrorHelper.CreateError (2105, Xamarin.Bundler.Errors.MT2105_C, provider.AsString (), arg.Type.FullName);
					return (BindingImplOptions) (int) arg.Value;
				default:
					throw ErrorHelper.CreateError (2105, Xamarin.Bundler.Errors.MT2105_D, provider.AsString (), ca.ConstructorArguments.Count);
				}
			}

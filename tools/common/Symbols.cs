	...
	public class Symbol
	{
		...
		public string Name {
			get {
				...
				throw ErrorHelper.CreateError (99, Xamarin.Bundler.Errors.MT0099, $"symbol without a name (type: {Type})");
			}
			set {
				name = value;
				...
				while ((line = reader.ReadLine ()) != null) {
					...
					if (line [0] == '\t') {
						...
						if (!target.Assemblies.TryGetValue (Assembly.GetIdentity (asm), out assembly))
							throw ErrorHelper.CreateError (99, Xamarin.Bundler.Errors.MT0099, $"serialized assembly {asm} for symbol {current.Name}, but no such assembly loaded");

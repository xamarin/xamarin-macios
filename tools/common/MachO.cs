...
namespace Xamarin
{
	...
	public class MachO
	{
		...
		public static IEnumerable<MachOFile> Read (string filename)
		{
			...
			if (fatfile != null) {
				...
				throw ErrorHelper.CreateError (1604, file.GetType ().Name, filename);
			}
		}
	...
	public class StaticLibrary
	{
		...
		internal void Read (string filename, BinaryReader reader, long size)
		{
			...
			while (reader.BaseStream.Position < pos + size) {
				...
				if (bytes [0] != 0x60 && bytes [1] != 0x0A)
					throw ErrorHelper.CreateError (1605, fileIdentifier, filename, bytes [0].ToString ("x"), bytes [1].ToString ("x"));
		...
		public static bool IsStaticLibrary (BinaryReader reader, bool throw_if_error = false)
		{
			...
			if (throw_if_error && !rv)
				throw ErrorHelper.CreateError (1601, System.Text.Encoding.ASCII.GetString (bytes, 0, 7));
	...
	public class MachOFile
	{
		...
		internal static bool IsMachOLibrary (FatEntry fat_entry, BinaryReader reader, bool throw_if_error = false)
		{
			...
			if (throw_if_error && !rv)
				throw ErrorHelper.CreateError (1600, magic.ToString ("x"), fat_entry.Parent.Filename);
		...
		internal void Read (BinaryReader reader)
		{
			...
			*/
			...
			case MachO.MH_MAGIC_64:
				...
				break;
			default:
				throw ErrorHelper.CreateError (1602, magic.ToString ("x"), fat_parent != null ? fat_parent.Parent.Filename : filename);
			}

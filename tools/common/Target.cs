	...
	public partial class Target {
		...
		public void ExtractNativeLinkInfo (List<Exception> exceptions)
		{
			...
			if (!App.OnlyStaticLibraries && Assemblies.Count ((v) => v.HasLinkWithAttributes) > 1) {
				ErrorHelper.Warning (127, Errors.mtouch.MT0127);
		...
		public static string GetRealPath (string path)
		{
			...
			ErrorHelper.Warning (54, Errors.mtouch.MT0054, path, FileCopier.strerror (errno), errno);
			return path;
		}
			...
			if (App.LinkMode != LinkMode.None) {
				foreach (Assembly assembly in Assemblies) {
					if ((assembly.AssemblyDefinition.MainModule.Attributes & ModuleAttributes.ILOnly) == 0)
						throw ErrorHelper.CreateError (2014, Errors.mtouch.MT2014, assembly.AssemblyDefinition.MainModule.FileName);
				}
			}
		}
		...
		public void GatherFrameworks ()
		{
			...
			if (asm == null)
				throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_E, Driver.GetProductAssembly (App));
					...
					if (Driver.GetFrameworks (App).TryGetValue (nspace, out framework)) {
						...
						case "PushKit":
							...
							if ((App.DeploymentTarget < v80) && (Driver.XcodeVersion < new Version (6, 2))) {
								ErrorHelper.Warning (49, Errors.mtouch.MT0049, framework.Name);
								continue;
							}
							break;
						case "WatchKit":
							...
							if (Driver.XcodeVersion.Major == 11 && App.Platform == ApplePlatform.iOS) {
								ErrorHelper.Warning (5219, Errors.mtouch.MT5219);
								continue;
							}
							break;
#endif
		...
		public void CollectAllSymbols ()
		{
			...
			foreach (var name in App.IgnoredSymbols) {
				...
				if (symbol == null) {
					ErrorHelper.Warning (5218, Errors.mtouch.MT5218, StringUtils.Quote (name));
				} else {
					symbol.Ignore = true;
				}
			}
		...
		bool IsRequiredSymbol (Symbol symbol, Assembly single_assembly = null)
		{
			...
			default:
				throw ErrorHelper.CreateError (99, Errors.mtouch.MT0099_F, symbol.Type, symbol.Name);
			}
		}

		...
		static void AddSharedOptions (Application app, Mono.Options.OptionSet options)
		{
			...
			options.Add ("marshal-objectivec-exceptions:", "Specify how Objective-C exceptions should be marshalled. Valid values: default, unwindmanagedcode, throwmanagedexception, abort and disable. The default depends on the target platform (on watchOS the default is 'throwmanagedexception', while on all other platforms it's 'disable').", v => {
				...
				case "disable":
					...
					break;
				default:
					throw ErrorHelper.CreateError (26, Errors.mtouch.MT0026, "--marshal-objective-exceptions", $"Invalid value: {v}. Valid values are: default, unwindmanagedcode, throwmanagedexception, abort and disable.");
				}
			});
			options.Add ("marshal-managed-exceptions:", "Specify how managed exceptions should be marshalled. Valid values: default, unwindnativecode, throwobjectivecexception, abort and disable. The default depends on the target platform (on watchOS the default is 'throwobjectivecexception', while on all other platform it's 'disable').", v => {
				...
				case "disable":
					...
					break;
				default:
					throw ErrorHelper.CreateError (26, Errors.mtouch.MT0026, "--marshal-managed-exceptions", $"Invalid value: {v}. Valid values are: default, unwindnativecode, throwobjectivecexception, abort and disable.");
				}
			});
			...
			options.Add ("dynamic-symbol-mode:", "Specify how dynamic symbols are treated so that they're not linked away by the native linker. Valid values: linker (pass \"-u symbol\" to the native linker), code (generate native code that uses the dynamic symbol), ignore (do nothing and hope for the best). The default is 'code' when using bitcode, and 'linker' otherwise.", (v) => {
				...
				case "ignore":
					...
					break;
				default:
					throw ErrorHelper.CreateError (26, Errors.mtouch.MT0026, "--dynamic-symbol-mode", $"Invalid value: {v}. Valid values are: default, linker, code and ignore.");
				}
			});
...
#pragma warning restore 0414
		...
		static void SetTargetFramework (string fx)
		{
			...
			default:
				...
				if (!Xamarin.Utils.TargetFramework.TryParse (fx, out parsedFramework))
					throw ErrorHelper.CreateError (68, Errors.mtouch.MT0068, fx);
#if MONOMAC
			...
			if (Array.IndexOf (TargetFramework.ValidFrameworks, targetFramework.Value) == -1)
				throw ErrorHelper.CreateError (70, Errors.mtouch.MT0070, targetFramework.Value, string.Join (" ", TargetFramework.ValidFrameworks.Select ((v) => v.ToString ()).ToArray ()));
#endif
		}
		...
		public static void WriteIfDifferent (string path, string contents, bool use_stamp = false)
		{
			...
			try {
				...
				File.WriteAllText (path, contents);
				ErrorHelper.Warning (1014, e, Errors.mtouch.MT1014, path, e.Message);
			} finally {
		...
		static void SetCurrentLanguage ()
		{
			...
			try {
				...
				ErrorHelper.Warning (124, e, Errors.mtouch.MT0124, lang, lang_variable, e.Message);
			}
		}
		...
		public static void Touch (IEnumerable<string> filenames, DateTime? timestamp = null)
		{
			...
			foreach (var filename in filenames) {
				try {
					...
					ErrorHelper.Warning (128, Errors.mtouch.MT0128, filename, e.Message);
				}
			}
		}
		...
		{
			if (!File.Exists (name))
				throw ErrorHelper.CreateError (24, Errors.mtouch.MT0024, name);
		...
		static string FindSystemXcode ()
		{
			...
			if (Driver.RunCommand ("xcode-select", new [] { "-p" }, output: output) != 0) {
				ErrorHelper.Warning (59, Errors.mtouch.MT0059, output.ToString ());
				return null;
			}
		...
		static void ValidateXcode (bool accept_any_xcode_version, bool warn_if_not_found)
		{
			if (sdk_root == null) {
				...
				if (sdk_root == null) {
					...
					ErrorHelper.Warning (60, Errors.mtouch.MT0060, sdk_root);
					sdk_root = null;
				} else {
					if (!accept_any_xcode_version)
						ErrorHelper.Warning (61, Errors.mtouch.MT0061, sdk_root);
				}
				if (sdk_root == null) {
					...
					if (!Directory.Exists (sdk_root)) {
						if (warn_if_not_found) {
							...
							ErrorHelper.Warning (56, Errors.mtouch.MT0056);
							return; // Can't validate the version below if we can't even find Xcode...
						}

						throw ErrorHelper.CreateError (56, Errors.mtouch.MT0056);
					}
					ErrorHelper.Warning (62, Errors.mtouch.MT0062, sdk_root);
				}
			} else if (!Directory.Exists (sdk_root)) {
				throw ErrorHelper.CreateError (55, Errors.mtouch.MT0055, sdk_root);
			}
			...
			if (File.Exists (Path.Combine (sdk_root, "Contents", "MacOS", "Xcode"))) {
				...
				throw ErrorHelper.CreateError (57, Errors.mtouch.MT0057, sdk_root);
			}
			...
			if (File.Exists (plist_path)) {
				...
				throw ErrorHelper.CreateError (58, Errors.mtouch.MT0058, Path.GetDirectoryName (Path.GetDirectoryName (DeveloperDirectory)), plist_path);
			}

			if (!accept_any_xcode_version) {
				if (min_xcode_version != null && XcodeVersion < min_xcode_version)
					throw ErrorHelper.CreateError (51, Errors.mtouch.MT0051, Constants.Version, XcodeVersion.ToString (), sdk_root, PRODUCT, min_xcode_version);

				if (XcodeVersion < SdkVersions.XcodeVersion)
					ErrorHelper.Warning (79, Errors.mtouch.MT0079, Constants.Version, XcodeVersion.ToString (), sdk_root, SdkVersions.Xcode, PRODUCT);
			}

	...
	public abstract class CoreResolver : IAssemblyResolver {
		...
		public virtual AssemblyDefinition Load (string fileName)
		{
			...
			try {
				...
				try {
					...
					ErrorHelper.Show (ErrorHelper.CreateWarning (129, fileName));
				}
			}
			catch (Exception e) {
				throw new ProductException (9, true, e, fileName);
			}
		...
		static string DirectoryGetFile (string directory, string file)
		{
			...
			if (files != null && files.Length > 0) {
				if (files.Length > 1) {
					ErrorHelper.Warning (133, file, Environment.NewLine, string.Join ("\n", files));
				}
				return files [0];
			}

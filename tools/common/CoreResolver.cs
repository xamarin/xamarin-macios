	...
	public abstract class CoreResolver : IAssemblyResolver {
		...
		public virtual AssemblyDefinition Load (string fileName)
		{
			...
			try {
				...
				try {
					...
					ErrorHelper.Show (ErrorHelper.CreateWarning (129, mtouch.Errors.MT0129, fileName));
				}
			}
			catch (Exception e) {
				throw new ProductException (9, true, e, mtouch.Errors.MT0009, fileName);
			}
		...
		static string DirectoryGetFile (string directory, string file)
		{
			...
			if (files != null && files.Length > 0) {
				if (files.Length > 1) {
					ErrorHelper.Warning (133, mtouch.Errors.MT0133, file, Environment.NewLine, string.Join ("\n", files));
				}
				return files [0];
			}
